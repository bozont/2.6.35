...
#if defined(CONFIG_S6D05A0_MSG_DEBUG)
enum {
	LCD_DEBUG_DATA = 1U << 6, /* All debug */
	LCD_DEBUG_FUNCTION = 1U << 7, /* All debug */
};

static unsigned int lge_lcd_debug_mask = 0;

module_param_named(debug_mask, lge_lcd_debug_mask, int,
		S_IRUGO | S_IWUSR | S_IWGRP);

#define KNIGHT_DBG(mask, fmt, args...) \
	do {\
		if ((mask) & lge_lcd_debug_mask) \
			printk(KERN_INFO "[MC] [%-18s:%5d] " \
					fmt, __func__, __LINE__, ## args);\
	} while(0)
#else
#define KNIGHT_DBG(mask, fmt, args...) do {} while(0)
#endif
...
static void display_table(struct display_table *table, unsigned int count)
{
	...
	KNIGHT_DBG(LCD_DEBUG_FUNCTION, "%s: count : %d\n", __func__, count);
	...
	for(i = 0; i < count; i++) {
		...
		default:
			mddi_host_register_cmds_write32(reg, table[i].count, table[i].val_list,
					0, 0, 0);
			KNIGHT_DBG(LCD_DEBUG_DATA, "reg : %x, val : 0x%X.\n",
					...
					void *arg)
{
	...
	KNIGHT_DBG(LCD_DEBUG_FUNCTION, "%s : handler = %x\n", 
...
static void mddi_ss_driveric_vsync_detected(boolean detected)
{
	...
	KNIGHT_DBG(LCD_DEBUG_FUNCTION, "%s : detected = %d\n", __func__, detected);
...
{
	KNIGHT_DBG(LCD_DEBUG_FUNCTION, "%s : Reset pin = %d\n",
...
{
	KNIGHT_DBG(LCD_DEBUG_FUNCTION, "%s : state = %d\n", __func__, ss_panel);
...
static int mddi_ss_driveric_on(struct platform_device *pdev)
{
   ...
   if (system_state != SYSTEM_BOOTING) {
	...
	KNIGHT_DBG(LCD_DEBUG_FUNCTION, "%s\n", __func__);
...
#if 0
	if (TM_GET_DID(mfd->panel.id) == SHARP)
		mddi_ss_driveric_innotek_init(mfd);
	else
		mddi_ss_driveric_sharp_init(mfd);
#endif
...
mddi_ss_driveric_set_backlight(struct msm_fb_data_type * mfd)
{
        ...
        printk(KERN_ERR "[sungwoo] bl_level : %d \n", mfd->bl_level);
...
#if defined(CONFIG_S6D05A0_SYS_DEBUG)
static int onoff_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d", ss_driveric_state);
}

static int onoff_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned int value;
	...
	if (value == 0)
		mddi_ss_driveric_display_off(NULL);
	else if (value == 1)
		mddi_ss_driveric_display_on(NULL);
	else
		printk("0 : off, 1 : on \n");
...
static int __devinit mddi_ss_driveric_probe(struct platform_device *pdev)
{
	int err;

	if (pdev->id == 0) {
		KNIGHT_DBG(LCD_DEBUG_FUNCTION, "%s : pdev->id = 0\n", __func__);
...
#if defined(CONFIG_S6D05A0_SYS_DEBUG)
	err = device_create_file(&pdev->dev, &dev_attr_onoff);
#endif
		...
		u32 channel, u32 panel)
{
	...
	pdev = platform_device_alloc("mddi_ss", (panel << 8)|channel);
	if (!pdev)
		return -ENOMEM;
...
int __init mddi_ss_driveric_init(void)
{
	KNIGHT_DBG(LCD_DEBUG_FUNCTION, "%s\n", __func__);
...
MODULE_DESCRIPTION("mddi samsung driver IC for Swift(MSM)");
MODULE_LICENSE("GPL");
